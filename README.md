...
- **문제** — 노드·엣지가 각 1만 개일 때 화면 이동(pan)·확대/축소(zoom)마다 전체 그래프를
  매 프레임 다시 그려 프레임이 끊겼다.
- **접근** — 추측 대신 측정부터. FPS 카운터(`useFps`)와 1만 노드 일괄 생성 dev
  도구(`seedTestGraph`)를 직접 구현해, 화면 밖 객체의 불필요한 그리기 호출이 병목임을
  특정했다.
- **해결**
  - 렌더 직전 뷰포트 world 영역을 한 번 계산하고, 경계 사각형이 교차하는 노드·엣지만
    그리는 뷰포트 컬링을 구현([viewport.ts](src/lib/canvas/viewport.ts)).
  - 연산 비용이 큰 베지어 곡선의 정확한 bound 계산 대신 **양 끝 노드를 포함하는 사각형으로
    근사**해 비용을 낮췄다.
  - `nodes 보임/전체` HUD로 화면 밖 객체가 실제로 제외되는지 실시간 확인.
- **결과** — pan/zoom·드래그 인터랙션 중에도 **60fps 이상 유지**(측정 67fps), 인터랙션이
  끝나면 모니터 상한(144Hz)으로 복귀해 누적 부담이 없음을 `useFps` HUD로 확인
  ([이슈 #19](https://github.com/mulddang2/mindcanvas/issues/19) 해결).

### 2. 실시간 멀티커서 동기화 — 원격 이벤트 rAF 쓰로틀링

- **문제** — 다중 접속 시 원격 커서 awareness 갱신 이벤트가 잦아 수신 측 리렌더가
  메인 스레드를 압박했다.
- **해결** — 대량의 커서 갱신을 `requestAnimationFrame`으로 묶어 **프레임당 1회**로
  제한([#79](https://github.com/mulddang2/mindcanvas/pull/79)). 이동 자체는 눈에 띄는
  손실 없이 갱신 빈도만 프레임 예산에 맞췄다.
- **결과** — 다중 커서 환경에서도 렌더 주기가 프레임당 1회로 안착해 불필요한 리렌더를 걷어냈다.

### 3. 무료 티어 배포 — Vercel + Render 분리와 pnpm 핀

- **문제** — Y.js WebSocket은 상시 연결이 필요해 Vercel 서버리스에 올리기 어렵고, Render
  무료 플랜 빌드가 corepack의 pnpm 버전 드리프트로 깨졌다.
- **해결**
  - 서비스 성격에 맞춰 **Next 앱은 Vercel, Y.js 릴레이만 Render**(무료 티어)로 분리 배포.
  - `package.json`에 `packageManager: pnpm@10.11.0`을 명시해 버전 드리프트를 막고, 릴레이
    빌드는 `--ignore-scripts`로 husky 등 불필요한 의존성 빌드를 건너뛰었다
    ([#87](https://github.com/mulddang2/mindcanvas/pull/87)).
  - Render 무료 티어의 7일 무활동 정지는 cron keep-alive 핑으로 방지
